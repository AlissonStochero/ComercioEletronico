				...
				<ul class="nav nav-tabs" id="grid-demo-tabs">
					...
					<li class="pull-right">
						...
						<select id="select_theme" class="input-medium">
							...
							<?php
							$black = array("metro-black","black-tie","dark-hive","dot-luv","trontastic","vader","ui-darkness");
							$white = array("base","metro-light","blitzer","south-street","start","cupertino","flick","hot-sneaks","redmond","smoothness");
						...
						Layout:
						<select id="select_layout" class="input-small">
		...
		<script>
			...
			var update_themes = function(){
				...
				switch(t)
				{
					...
					default:
						$("iframe[name=demo_frame]").contents().find("body").append('<link rel="stylesheet" type="text/css" media="screen" href="vendor/php_grid_lib/js/themes/'+t+'/jquery-ui.custom.css"></link>');
				}

				if ( $("iframe[name=demo_frame]")[0].contentWindow.location.href.indexOf('bootstrap') != -1)
					$("iframe[name=demo_frame]").contents().find("body").append('<link rel="stylesheet" type="text/css" media="screen" href="vendor/php_grid_lib/js/jqgrid/css/ui.jqgrid.bs.css"></link>');
				else
					$("iframe[name=demo_frame]").contents().find("body").append('<link rel="stylesheet" type="text/css" media="screen" href="vendor/php_grid_lib/js/jqgrid/css/ui.jqgrid'+l+'.css"></link>');
